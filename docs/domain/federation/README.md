# Contesto: federation

**Stato modellazione:** `scheletro`

Vedi [bounded-contexts.md](../bounded-contexts.md) e [metodo dominio](../README.md).

## File da compilare

| File | Contenuto |
|------|-----------|
| `glossary.md` | Linguaggio ubiquo |
| `commands-and-events.md` | Comandi, eventi, invarianti (Event Storming) |

## UML

`docs/model/uml/federation/` — `federation-state.puml`, `seq-*.puml`

## Statechart (se UI)

`client/lib/machines/federation/` — vedi [client/lib/machines/README.md](../../../client/lib/machines/README.md)
