...
Per una conversazione commessa con archive_user `O` e peer `P` (`peer_address`):

1. JWT GoTrue presente sul client dell'account `O`
2. `auth.uid() == O`
3. `P` ≠ indirizzo dell'account `O`
...
1. Se il `peer_address` richiesto ≠ peer in view-state, la chat stale non resta commessa (`ConversationScopeInvalidated`).
...
1. Solo `NavigationMachine.commitScope` registra `ConversationScope` con `(focus_user_id, peer_address, session_epoch)`; `invalidateCommittedScope` lo azzera.
...
4. Push e deep link: keyed su `peerAddress` — nessun dual-read `peerProfileId`.
